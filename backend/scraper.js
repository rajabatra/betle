...
async function checkGames(leagueUrl, date) {
  ...
  switch (leagueUrl) {
    case "https://v1.american-football.api-sports.io":
      gameChoicesJson = responseJson.response.filter(
        (x) => parseInt(x.game.date.time.split(":")[0]) <= 4,
      );
      break;
    case "https://v2.nba.api-sports.io":
      gameChoicesJson = responseJson.response.filter(
        (x) => parseInt(x.date.start.substring(11, 13)) <= 4,
      );
      break;
    case "https://v1.baseball.api-sports.io":
      gameChoicesJson = responseJson.response.filter(
        (game) => game.time < "04:00",
      );
      break;
    case "https://v3.football.api-sports.io":
      gameChoicesJson = responseJson.response.filter(
        (x) => parseInt(x.fixture.date.substring(11, 13)) <= 4,
      );
      break;
  }
...
async function generateGame() {
  ...
  while (
    !(await checkGames(leagueChoice, gameDate)) &&
    i < leagues.length - 1
  ) {
    i++;
  ...
  const gameJson =
    gameChoicesJson[Math.floor(Math.random() * gameChoicesJson.length)];
  let gameInfo = {};
